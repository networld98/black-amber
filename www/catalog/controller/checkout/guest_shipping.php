...
class ControllerCheckoutGuestShipping extends Controller {
	public function index() {
        ...
        if($this->session->data['shipping_method']['code']=='flat.flat') {
            $data['city'] = $this->session->data['payment_address']['address_2'];
        }
...
//		}
		...
		if (!$json) {
            echo "<pre>";
            print_r($this->request->post['city']);
            echo "</pre>";
            ...
            if($this->session->data['shipping_method']['code']=='flat.flat') {
                $this->session->data['shipping_address']['zone_id'] = $this->request->post['postcode'];
            }
			$this->load->model('localisation/country');
