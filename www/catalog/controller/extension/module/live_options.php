...
class ControllerExtensionModuleLiveOptions extends Controller {
	...
	public function __construct($params) {
		...
		$this->status        		   = $this->config->get('module_live_options_status');
	...
	public function index() {
		...
		if ($product_info) {
			...
			$cache_key = 'module_live_options_'. md5($product_id . $quantity. $options_hash . $this->session->data['currency'] . $this->session->data['language'] . $this->show_type . $this->show_option_type . $this->calculate_quantity);
			if (!$this->use_cache || (!$json = $this->cache->get($cache_key))) {
				...
				if($this->show_type){
					...
					$options_makeup = ($options_makeup == 0) ? '' : ' ('.$prefix_makeup.$this->currency->format($options_makeup, $this->session->data['currency']).')';
					$options_makeup_notax = ($options_makeup_notax == 0) ? '' : ' ('.$prefix_makeup.$this->currency->format($options_makeup_notax, $this->session->data['currency']).')';

					if ($this->data['price']) {
						$json['new_price']['price'] = $this->currency->format($this->data['price'], $this->session->data['currency']) . $options_makeup;
					} else {
						$json['new_price']['price'] = false;
					}
					if ($this->data['special']) {
                        $percent = 100 / $product_info['price'] * $product_info['special'];
                        $json['new_price']['special'] = $this->currency->format($this->data['price'] * $percent / 100, $this->session->data['currency']) . $options_makeup;
					} else {
						$json['new_price']['special'] = false;
					}
					if ($this->config->get('config_tax')) {
						$json['new_price']['tax'] = $this->currency->format(((float)$product_info['special'] ? $product_info['special'] : $product_info['price']), $this->session->data['currency'] ) . $options_makeup_notax;
					} else {
				...
				// Show total price
					...
					if ($this->data['price']) {
						$json['new_price']['price'] = $this->currency->format(($this->data['price'] + $options_makeup), $this->session->data['currency']);
					} else {
						$json['new_price']['price'] = false;
					}
					...
					if ($this->data['special']) {
                        $percent = 100 / $product_info['price'] * $product_info['special'];
						$json['new_price']['special'] = $this->currency->format(($this->data['price'] + $options_makeup) * $percent / 100, $this->session->data['currency']);
					} else {
						$json['new_price']['special'] = false;
					}

					if ($this->config->get('config_tax')) {
						$json['new_price']['tax'] = $this->currency->format(((float)$product_info['special'] ? ($product_info['special'] + $options_makeup) : ($product_info['price'] + $options_makeup_notax)), $this->session->data['currency'] );
					} else {
						$json['new_price']['tax'] = false;
					}
				}

				$json['new_price']['reward'] = ($product_info['reward'] > 0) ? (abs($product_info['reward'])) : 0;
				$json['new_price']['points'] = ($product_info['points'] + $options_points > 0) ? abs($product_info['points'] + $options_points) : 0;
